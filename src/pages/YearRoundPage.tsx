...
const HERO_IMAGE = hotelGallery.palmCanopyEntrance;
...
const CUSTOM_IMAGE = hotelGallery.luxuryVillaGardenPathway;
  ...
  return (
    <div className="bg-surface select-text">
      <section className="relative min-h-[64vh] md:min-h-[72vh] w-full flex items-end md:items-center overflow-hidden">
        ...
        <div className="absolute inset-0 z-[1] pointer-events-none" style={{ background: "rgba(0,0,0,0.18)" }} aria-hidden />
      ...
      <FeatureSection copy={copy.dining} image={DINING_IMAGE} language={language} reversed />
      <FeatureSection copy={copy.jewishLife} image={JEWISH_IMAGE} language={language} muted />
      <FeatureSection copy={copy.excursions} image={EXCURSIONS_IMAGE} language={language} reversed />
      <FeatureSection copy={copy.families} image={FAMILIES_IMAGE} language={language} muted />
      <FeatureSection copy={copy.custom} image={CUSTOM_IMAGE} language={language} reversed />

      <section id="year-round-inquiry" className="py-14 md:py-28 px-5 sm:px-6 md:px-margin-desktop max-w-container-max mx-auto">
